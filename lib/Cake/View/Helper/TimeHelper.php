...
/**
 * Returns a friendly string describing the duration in time ago something happened based on $seconds
 * 
 *
 * @param num $seconds
 * @return friendly worded string
 * 
 */
	...
	public function friendlyTime($seconds){
		...
		{
		  return round($seconds/$MINUTE) . " minutes ago";
		}
		...
		if ($seconds < 24 * $HOUR)
		{
		  return round($seconds/$HOUR) . " hours ago";
		}
		if ($seconds < 48 * $HOUR)
		{
		  return "yesterday";
		...
		if ($seconds < 30 * $DAY)
		{
		  return round($seconds/$DAY) . " days ago";
		}
		if ($seconds < 12 * $MONTH)
		{
